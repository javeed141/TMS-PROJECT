...
export default function EventModal({ open, onClose, initialValues = {}, onSave, isDark = false }) {
  ...
  async function createAndAddTasks(payload) {
    const API = "https://tms-project-8.onrender.com/api/meetings/create-and-addtasks";
  ...
  async function checkAvailability(email) {
    const API = "https://tms-project-8.onrender.com/api/executive/check-availability";
    const payload = {
      email,
  ...
  const logConflictTicket = async (guest) => {
    const API = "https://tms-project-8.onrender.com/api/meetings/conflicts/manual";
